...
static inline bool have_movdir64b(void)
{
	...
	if (!__get_cpuid_count(7, 0, &ax, &bx, &cx, &dx))
		return 0;
...
static inline void ionic_movdir64b_mmio_memcpy_x64_64(void __iomem *dst, const void *src)
{
...
static inline void ionic_movdir64b_mmio_memcpy_x64(void __iomem *dst, const void *src,
						   size_t bytecnt)
{
	/*
	 * Caller must guarantee:
...
static inline void ionic_mmio_memcpy_x64_64(void __iomem *dst, const void *src)
{
#ifdef __x86_64__
...
static inline void ionic_mmio_memcpy_x64(void __iomem *dst, const void *src, size_t bytecnt)
{
#ifdef __x86_64__
	if (likely(ionic_have_movdir64b))
...
static struct ibv_pd *ionic_alloc_pd(struct ibv_context *ibctx)
{
	...
	if (ctx->ovrd_sq_cmb != -1) {
		pd->sq_cmb = ctx->ovrd_sq_cmb;
	} else {
	...
	if (ctx->ovrd_rq_cmb != -1) {
		pd->rq_cmb = ctx->ovrd_rq_cmb;
	} else {
...
static void ionic_vcq_cq_deinit(struct ionic_cq *cq)
{
	...
	if (ionic_g.init) {
		pthread_mutex_lock(&ionic_g.mut);
		list_del(&cq->g_ent);
		pthread_mutex_unlock(&ionic_g.mut);
	}
...
static void ionic_vcq_cq_init2(struct ionic_cq *cq, uint32_t resp_cqid)
{
	...
	if (ionic_g.init) {
		pthread_mutex_lock(&ionic_g.mut);
		list_add_tail(&ionic_g.cq_list, &cq->g_ent);
		pthread_mutex_unlock(&ionic_g.mut);
	}
}
			   ...
			   struct ibv_wc *wc)
{
	...
	/* wqe_id must indicate a request that is outstanding */
	meta = &qp->rq_meta[qp->rq_meta_idx[cqe->recv.wqe_id]];
	if (unlikely(meta->next != IONIC_META_POSTED)) {
		ionic_err("wqe is not posted for idx %lu meta_idx %u qpid %u rq.prod %u rq.cons %u cqid %u",
			  (unsigned long)cqe->recv.wqe_id,
			  qp->rq_meta_idx[cqe->recv.wqe_id],
			  qp->qpid, qp->rq.prod, qp->rq.cons, cq->cqid);
		return -EIO;
	}
	...
	if (ionic_v1_cqe_error(cqe)) {
		...
		ionic_err("cqe with error %u for %#x (recv), qpid %u cqid %u",
			  wc->status, be32toh(cqe->send.msg_msn), qp->qpid, cq->cqid);
			  ...
			  struct ionic_v1_cqe *cqe)
{
	...
	if (rc) {
		...
		ionic_err("wqe is not posted for %#x (msn), qpid %u cqid %u seq %u for prod %u cons %u\n",
	...
	if (ionic_v1_cqe_error(cqe)) {
		...
		ionic_err("cqe with error %u for %#x (msn), qpid %u cqid %u",
			  meta->ibsts, be32toh(cqe->send.msg_msn), qp->qpid, cq->cqid);
	}
			  ...
			  struct ionic_v1_cqe *cqe)
{
	...
	if (ionic_v1_cqe_error(cqe)) {
		...
		meta->remote = false;
		ionic_err("cqe with error %s for %#x (npg), qpid %u cqid %u",
			  ibv_wc_status_str(meta->ibsts), be32toh(cqe->send.msg_msn),
			  qp->qpid, cq->cqid);
	}
			     ...
			     int nwc, struct ibv_wc *wc)
{
	...
	while (likely(ionic_next_cqe(ctx, cq, &cqe))) {
		...
		if (unlikely(!qp))
		{
			ionic_err("cq %d pos %d missing qp for qid %#x",
			    ...
			    int max_wr, int max_sge, int max_data)
{
	...
	rc = ionic_queue_init(&qp->sq, pd, IONIC_PD_TAG_SQ,
			    ...
			    int max_wr, int max_sge)
{
	...
	if (qp->rq_cmb & IONIC_CMB_EXPDB) {
		wqe_size = ionic_v1_recv_wqe_min_size(max_sge, qp->rq_spec, true);
	...
	if (!(qp->rq_cmb & IONIC_CMB_EXPDB))
		wqe_size = ionic_v1_recv_wqe_min_size(max_sge, qp->rq_spec, false);

	rc = ionic_queue_init(&qp->rq, pd, pd_tag, ctx->pg_shift, max_wr, wqe_size);
	if (rc)
		goto err_rq;
	...
	qp->rq_meta_idx = calloc((uint32_t)qp->rq.mask + 1,
				 sizeof(*qp->rq_meta_idx));
	if (!qp->rq_meta_idx) {
		rc = ENOMEM;
					 ...
					 struct ibv_qp_init_attr_ex *ex)
{
	...
	qp->sq_cmb = resp.sq_cmb;
	qp->rq_cmb = resp.rq_cmb;
	...
	if (qp->sq_cmb & IONIC_CMB_ENABLE) {
		...
		if (qp->sq_cmb & IONIC_CMB_EXPDB) {
			...
			wqe_sz = 1 << qp->sq.stride_log2;
			db_ptr = qp->sq.ptr + wqe_sz - IONIC_EXP_DBELL_SZ;

			/* Assume ring 0 */
			db_base = ((uint64_t)ctx->sq_qtype << 48) |
				qp->rq.dbell |
				((0x03 | 0x8) << 19); /* SCHED_SET | PICI_PISET */

			for (pos = 0; pos <= qp->sq.mask; pos++) {
				db_data = db_base | ((pos + 1) & qp->sq.mask);
				*(uint64_t *)db_ptr = htole64(db_data);
	...
	if (qp->rq_cmb & IONIC_CMB_ENABLE) {
		...
		if (qp->rq_cmb & IONIC_CMB_EXPDB) {
			/* Pre-fill express doorbells into our buffer */
			wqe_sz = 1 << qp->rq.stride_log2;
			db_ptr = qp->rq.ptr + wqe_sz - IONIC_EXP_DBELL_SZ;

			/* Assume ring 0 */
			db_base = ((uint64_t)ctx->rq_qtype << 48) |
				qp->rq.dbell |
				((0x03 | 0x8) << 19); /* SCHED_SET | PICI_PISET */

			for (pos = 0; pos <= qp->rq.mask; pos++) {
				db_data = db_base | ((pos + 1) & qp->rq.mask);
				*(uint64_t *)db_ptr = htole64(db_data);
	...
	list_node_init(&qp->g_ent);
	if (ionic_g.init) {
		pthread_mutex_lock(&ionic_g.mut);
		list_add_tail(&ionic_g.qp_list, &qp->g_ent);
		pthread_mutex_unlock(&ionic_g.mut);
	}
...
err_cmd:
	if (attr_mask & IBV_QP_STATE)
		ionic_dbg(ctx, "modify qp %u state %u -> %u rc %d",
			  ...
			  struct ibv_qp_init_attr *init_attr)
{
	...
	if (qp->has_rq) {
		...
		init_attr->cap.max_recv_sge =
			ionic_v1_send_wqe_max_sge(qp->rq.stride_log2, qp->rq_spec,
						  qp->rq_cmb & IONIC_CMB_EXPDB);
	}

	attr->cap = init_attr->cap;

	ionic_dbg(ctx, "query qp %u attr_state %u rc %d", qp->qpid, attr->qp_state, rc);
			       ...
			       struct ionic_v1_wqe *wqe)
{
	...
	qp->sq_color = ionic_color_wrap(qp->sq.prod, qp->sq_color);
}
...
{
	uint32_t wqe_sz = 1u << qp->sq.stride_log2;
...
{
	uint32_t wqe_sz = 1u << qp->rq.stride_log2;
	...
	if (qp->sq_cmb & IONIC_CMB_EXPDB) {
		while (pos != end) {
			...
			do {
				...
				if (cmb_wc && stride <= 64)
					mmio_flush_writes();
				ionic_mmio_memcpy_x64_64(cmb_ptr, wqe_ptr);
		...
		if (pos > end) {
			cmb_ptr = qp->sq_cmb_ptr + ((size_t)pos << stride_log2);
			wqe_ptr = ionic_queue_at(&qp->sq, pos);
			stride = (uint32_t)(qp->sq.mask - pos + 1) << stride_log2;

			ionic_mmio_memcpy_x64(cmb_ptr, wqe_ptr, stride);
...
static void ionic_post_recv_cmb(struct ionic_ctx *ctx, struct ionic_qp *qp)
{
	...
	if (qp->rq_cmb & IONIC_CMB_EXPDB) {
		while (pos != end) {
			...
			pos = ionic_queue_next(&qp->rq, pos);
		}
	} else {
		if (pos > end) {
			cmb_ptr = qp->rq_cmb_ptr + ((size_t)pos << stride_log2);
			      ...
			      struct ibv_recv_wr *wr)
{
	...
	mval = ionic_v1_recv_wqe_max_sge(qp->rq.stride_log2,
					 qp->rq_spec, qp->rq_cmb & IONIC_CMB_EXPDB);
	...
	qp->rq_meta_idx[qp->rq.prod] = meta - qp->rq_meta;
